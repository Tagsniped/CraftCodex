export const slimefunHeadTextures = {
...
export function headTextureFromStatement(statement, args = []) {
  const headTexture = statement.match(/\bHeadTexture\.([A-Z0-9_]+)/)?.[1];
  if (headTexture && slimefunHeadTextures[headTexture]) return slimefunHeadTextures[headTexture];

  const directHash = args.find((arg, index) => index > 0 && /^"[a-f0-9]{32,}"$/i.test(arg.trim()));
  if (directHash) return directHash.trim().slice(1, -1);

  return "";
}

export function spriteFromMaterialOrHead(statement, args, material) {
  const texture = headTextureFromStatement(statement, args);
  if (texture) return { type: "player_head", texture };
  return { type: "item", id: material ? material.toLowerCase() : "player_head" };
}
